...
int nAnonymizeMoneyByteAmount = 1000;
...
#else
    const char* pszModule = "MoneyByte";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MoneyByte
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MoneyByte
    // Mac: ~/Library/Application Support/MoneyByte
    // Unix: ~/.MoneyByte
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MoneyByte";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "MoneyByte.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "MoneyByted.pid"));
