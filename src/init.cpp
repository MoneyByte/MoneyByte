...
void Shutdown()
{
    ...
    RenameThread("MoneyByte-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: MoneyByte.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: MoneyByted.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizemoneybyteamount=<n> " + _("Keep N MoneyByte anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MoneyByte is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MoneyByte is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("MoneyByte version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "MoneyByte server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MoneyByte") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart MoneyByte to complete") << "\n";
    ...
    nAnonymizeMoneyByteAmount = GetArg("-anonymizemoneybyteamount", 0);
    if(nAnonymizeMoneyByteAmount > 999999) nAnonymizeMoneyByteAmount = 999999;
    if(nAnonymizeMoneyByteAmount < 2) nAnonymizeMoneyByteAmount = 2;
    ...
    LogPrintf("Anonymize MoneyByte Amount %d\n", nAnonymizeMoneyByteAmount);
